...
3. Scan: data is acquired, displayed, and appended to stream file for each stage position
  - Data is saved to same HDF5 file (e.g. filename.h5) in same folder as Scan.py:
  - If no stage is chosen, a single shot/trace is recorded 
...
-c --channel=
        define the channel that willb e used to acquire data.
...
def initializePolytec(portPolytec, baudPolytec, decoder, drange):
    '''Initialize Polytec vibrometer and obtain relevant settings to save in trace headers'''
    # set decoder range
