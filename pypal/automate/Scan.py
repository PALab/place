'''
Program to run laser-ultrasound experiments using pyPAL.

1. Instruments are initialized
2. Header is created
3. Scan: trace is acquired, plotted and saved to stream file at each stage position.
   Data is saved to same folder as Scan.py:
       -Image data is saved to HDF5 file, e.g. 'filename.h5' 
...
from pypal.automate import osci_card, xps_control, tektronix, polytec
exit()
def main():
...
def scan(channel, GroupName, xps, socketId, xi, dx, xf, totalTraces, mapColor, filename, header):   
    ...
    if xi <= xf:
        ...
        while x < (xf+dx):  
            ...
            c = 0
            while xps.GroupPositionCurrentGet(socketId, GroupName,3) != x: # make sure stage actually moved
                xps.GroupMoveAbsolute(socketId, GroupName, [x])
                if c > 3: 
