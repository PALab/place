...
class Setup:
    ...
    def closeConnection(self):
        '''
        Close serial connection to DS345
        '''
        self.fgen.close()
...
class Modulate(Setup):
    ...
    def setModWaveform(self,modType='single'):
        ...
        **NOTE: 'arb' = arbitrary waveform, and may only be set for AM, FM< and PM.
        '''
        print modType
        if modType == 'single':
...
class Arbitrary(Setup):
    ...
    def loadArbWaveform(self,datafile):
        ...
        fgen.write('ldwf? 0'+',' + str(lengthData) +' \n')
        sleep(1)
        reply = fgen.readline()
        ...
        if atoi(reply) == 1:
            ...
            for iDataItem in range(lengthData):
                fgen.write(pack('h',data[iDataItem]))
    ...
    def loadModulationPattern(self,datafile):
        ''' 
        Download arbitrary modulation patterns.  
        Modulation type must already be set to AM, FM, or PM. See  Modulate().setModType() 
        ...
        # Ask for data load permission, fgen returns '1' if it is ready
        self.fgen.write('amod? 0'+',' + str(lengthData) +' \n')
        sleep(1)
        reply = fgen.readline()
