...
class QuantaRay(Instrument):
    ...
    def setWatchdog(self, time=10):
        """Set range of watchdog.

        If the laser does not receive communication from the control computer
        within the specifiedc time, it turns off.  If disabled, the default
        time is zero.  time must be between 0 and 110 seconds.
        """
    ...
    def setBaud(self, baudINDI=9600):
        """Sets baudrate of laser.

        At power-up, baudrate is always 9600.
        """
    ...
    def getAmpPower(self):
        """Queries amplifier PFN monitor in percent (what PFN power supply is
        actually doing)"""
    ...
    def getTrigRate(self):
        """Queries and returns the lamp trigger rate (unless lamp trigger
        source is external
        """
    ...
    def getStatus(self):
        ...
        if STB[len(STB)-16] == '1':
            bit = '15'
            error = ('Interlock open: CDRH plug, power supply cover, laser head ' +
                     'cover, laser head temperature, water pressure, or water flow')
            stat = [bit, error]
            errorList.append(stat)
        if STB[len(STB)-17] == '1':
            bit = '16'
